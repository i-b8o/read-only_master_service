...
func NewRegulationUsecase(regulationService RegulationService, chapterService ChapterService, paragraphService ParagraphService, linkService LinkService, speechService SpeechService, absentService AbsentService, logging logging.Logger) *regulationUsecase {
	return &regulationUsecase{regulationService: regulationService, chapterService: chapterService, paragraphService: paragraphService, linkService: linkService, speechService: speechService, absentService: absentService, logging: logging}
}
...
func (u regulationUsecase) DeleteRegulation(ctx context.Context, regulationID uint64) error {
	...
	status, err := u.regulationService.DeleteRegulation(ctx, regulationID)
	if err != nil {
		u.logging.Error(status)
		return err
	}
