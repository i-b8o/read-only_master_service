...
func TestUpdate(t *testing.T) {
	...
	tests := []struct {
		input *pb.UpdateParagraphRequest
		err   error
	}{
		{
			input: &pb.UpdateParagraphRequest{ID: 3, Content: "Измененное содержимое третьего параграфа"},
			err:   nil,
		},
	}
...
func TestCreate(t *testing.T) {
	...
	for _, test := range tests {
		_, err := client.Create(ctx, test.input)
		if err != nil {
			t.Log(err)
		}
