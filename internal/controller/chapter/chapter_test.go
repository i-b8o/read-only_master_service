...
func TestCreate(t *testing.T) {
	...
	client := pb.NewMasterChapterGRPCClient(conn)
	defer conn.Close()
	...
	tests := []struct {
		...
		{
			input:    &pb.CreateChapterRequest{PseudoId: "3e3f0fd026f164a6d73563d9e437e7549ffaa924", DocId: 1, ChapterName: "Имя главы", ChapterNum: "V", OrderNum: 4},
	...
	for _, test := range tests {
		...
		assert.True(proto.Equal(test.expected, resp), fmt.Sprintf("CreateDoc(%v)=%v want: %v", test.input, resp, test.expected))
		...
		assert.Equal(test.input.DocId, regId)
		assert.Equal(test.input.ChapterName, name)
		assert.Equal(test.input.OrderNum, orderNum)
		assert.Equal(test.input.ChapterNum, num)
