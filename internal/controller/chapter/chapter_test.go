...
func TestCreate(t *testing.T) {
	...
	for _, test := range tests {
		...
		assert.True(proto.Equal(test.expected, resp), fmt.Sprintf("CreateRegulation(%v)=%v want: %v", test.input, resp, test.expected))
		...
		assert.Equal(test.input.RegulationId, regId)
